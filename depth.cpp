...
void depth_callback(const sensor_msgs::ImageConstPtr& msg,cv::Mat& depth_mask)
{
    ...
    // cv::dilate(depth_mask,depth_mask,getStructuringElement(MORPH_RECT, Size(13, 13)));
	// imshow("raw_image", depth_mask);
    // cv::resize(depth_image, depth_image, cv::Size(), 0.1, 0.1);
    ...
    //         // if(j==0)
    //         // cout<<endl;
    //         if(int(depth_image.ptr<float>(i)[j])<0)
    //             depth_image.ptr<float>(i)[j] = 0;
    //         // cout<<float(depth_image.ptr<float>(i)[j])<<' ';
...
void imageCallback(const sensor_msgs::ImageConstPtr& msg, cv::Ptr<cv::BackgroundSubtractor> pMOG2,int* frame_number, ros::Publisher& res_pub, Point& last_thumb,Point& last_center, double& last_angle,cv::Mat &depth_mask)
{  
    imshow("raw_image", depth_mask);
...
int main(int argc, char **argv)
{
    ...
    image_transport::Subscriber sub = it.subscribe("/image_raw", 1, boost::bind(&imageCallback, _1,pMOG2,&frame_number,res_pub,last_thumb,last_center,last_angle,depth_mask));
